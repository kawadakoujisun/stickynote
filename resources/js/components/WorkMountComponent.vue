        ...
        mounted() {
            ...
            window.Echo.private('sticky-note-import-channel.' + window.laravel.user['id'])
                .listen('StickyNoteImport', response => {
                    ...
                    // Pusherで送るデータが大き過ぎると
                    // local.ERROR: Pusher error: The data content of this event exceeds the allowed maximum (10240 bytes).
                    // See https://pusher.com/docs/channels/server_api/http-api#publishing-events for more info.
                    // というエラーになってしまうので、
                    // Pusherでデータを送らずにgetでデータベースからデータを取得してくるようにした。
                    
                    axios.get(window.laravel.asset + '/api/work-mount')
                        .then(response => {
                            console.log('axios.get');
                            
                            const importStickerParams = response.data;
